...
const useIPWhitelist = (middlewareName: string) => {
    ...
    const {control, formState, setValue, handleSubmit, reset, resetField} = useForm<{ip: string; name?: string}>({
        defaultValues: {
            ip: '',
            name: '',
        },
    ...
    const onSubmit: SubmitHandler<{ip: string; name?: string}> = useCallback(
        async (data) => {
            try {
                const result = await createWhitelistIPAction(middlewareName, data.ip, data.name);
