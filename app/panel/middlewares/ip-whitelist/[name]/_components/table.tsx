...
import useIPWhitelist from '../_hook/ip-whitelist.hook';
...
const TableIP: FC<Props> = ({name}) => {
    const {list, meta, control, formState, isOpen, handleSubmit, reset, onOpen, onOpenChange, onSubmit, onDelete} = useIPWhitelist(name);

    const renderCell = useCallback((data: {id: string; ip: string; name: string | null}, columnKey: React.Key): React.ReactElement => {
        switch (columnKey) {
            case 'ip':
                return (
        ...
        <div className='flex w-full flex-col gap-4'>
            ...
            >
                ...
                <TableBody
                    ...
                    items={list.items as {id: string; ip: string; name: string | null}[]}
                                    ...
                                    <div className='flex w-full flex-col gap-4'>
                                        ...
                                        <Controller
                                            ...
                                            render={({field, fieldState}) => (
                                                <Input
                                                    {...field}
                                                    description='Note: use descriptive name for your IP Whitelist.'
                                                    errorMessage={fieldState.error?.message}
                                                    isInvalid={fieldState.invalid}
                                                    isRequired={false}
                                                    label='IP Whitelist Name'
