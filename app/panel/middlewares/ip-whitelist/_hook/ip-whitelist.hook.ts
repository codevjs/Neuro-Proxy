...
import {createMiddlewareAction, deleteMiddlewareAction, getAllMiddlewareAction} from '../_action/action';
...
const useBasicAuthMiddleware = () => {
    ...
    const {control, formState, setValue, handleSubmit, reset, resetField} = useForm<{name: string}>({
        defaultValues: {
            name: '',
        },
    });
    ...
    const onSubmit: SubmitHandler<{name: string}> = useCallback(
        async (data) => {
            try {
                const result = await createMiddlewareAction(data.name);
...
export default useBasicAuthMiddleware;
